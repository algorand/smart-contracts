...
class AtomicTransactionComposer {
     ...
     */
    addMethodCall({ appId, method, methodArgs, sender, suggestedParams, onComplete, note, lease, rekeyTo, signer, }) {
        if (this.status !== AtomicTransactionComposerStatus.BUILDING) {
            throw new Error('Cannot add transactions when composer status is not BUILDING');
        }
        ...
        for (let i = 0; i < methodArgs.length; i++) {
            ...
            if (typeof argSpec.type === 'string' &&
                abi_1.abiTypeIsTransaction(argSpec.type)) {
                if (!signer_1.isTransactionWithSigner(argValue) ||
                    argSpec.type !== argValue.txn.type) {
                    throw new Error(`Expected ${argSpec.type} transaction for argument at index ${i}`);
                }
        ...
        if (appArgTypes.length > 14) {
            ...
            appArgTypes.push(new abi_1.ABITupleType(lastArgTupleTypes));
