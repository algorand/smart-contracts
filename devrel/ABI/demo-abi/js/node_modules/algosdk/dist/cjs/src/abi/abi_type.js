...
class ABIUintType extends ABIType {
    ...
    equal(other) {
        return (this.constructor === other.constructor && this.bitSize === other.bitSize);
    }
    isDynamic() {
        return false;
    }
...
class ABIUfixedType extends ABIType {
    ...
    encode(value) {
        if ((typeof value !== 'bigint' && typeof value !== 'number') ||
            value >= BigInt(2 ** this.bitSize) ||
            value < BigInt(0)) {
            throw new Error(`${value} is not a non-negative int or too big to fit in size ufixed${this.bitSize}`);
        }
        if (typeof value === 'number' && !Number.isSafeInteger(value)) {
            throw new Error(`${value} should be converted into a BigInt before it is encoded`);
        }
...
class ABIStringType extends ABIType {
    ...
    decode(byteString) {
        ...
        if (byteLength !== byteValue.length) {
            throw new Error(`string length bytes do not match the actual length of string: ${byteString}`);
        }
...
class ABITupleType extends ABIType {
    ...
    encode(values) {
        if (values.length > exports.MAX_LEN) {
