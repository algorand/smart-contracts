...
APPID=$(${gcmd} app create --creator ${ACCOUNT}   --approval-prog ./createasa.teal --global-byteslices 0 --global-ints 1 --local-byteslices 0 --local-ints 0  --clear-prog ./clear.teal | grep Created | awk '{ print $6 }')
echo "App ID="$APPID 

APPADDR=$(${gcmd} app info --app-id=$APPID | grep account: | awk '{ print $3 }')
echo "App ADD="$APPADDR 
${gcmd} clerk send --from=$ACCOUNT --to=$APPADDR --amount=5000000
